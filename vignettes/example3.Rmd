...
```{r}
foo <- function(x){
  ans1 <- repayment(house_value * (1 - down_payment_percent),               
                    beta = mortgage_rate / 12,     
                    m = mortage_year * 12) * mortage_year
    
  ans2_a <- house_value * (1 - down_payment_percent) * 0.01 * x
  ans2_b <- repayment(house_value * (1 - down_payment_percent),               
...
```{r}
uniroot(foo, interval = c(0, 30))$root
```
